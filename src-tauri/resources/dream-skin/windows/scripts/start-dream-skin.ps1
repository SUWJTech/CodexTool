...
function Test-DreamSkinRenderedVerificationOutput {
  ...
  try {
    ...
    foreach ($target in @($payload.targets)) {
      ...
      $foregroundDocument = $readiness.documentPass -is [bool] -and
        [bool]$readiness.documentPass
      # Current Codex builds report the document as hidden while CodexTool owns
      # the foreground during one-click apply. The native-window CDP binding is
      # also unavailable in that runtime even though the adopted stylesheet and
      # complete shell geometry are already present. Accept only that narrow
      # background-window case; identity, theme, viewport and structure checks
      # still have to pass.
      $verifiedBackgroundWindow = $result.nativeWindow.unsupported -is [bool] -and
        [bool]$result.nativeWindow.unsupported -and
        $readiness.fallbackWindowPass -is [bool] -and
        [bool]$readiness.fallbackWindowPass -and
        $result.shell.visible -is [bool] -and [bool]$result.shell.visible -and
        $result.composer.visible -is [bool] -and [bool]$result.composer.visible
      if ($result.installed -is [bool] -and [bool]$result.installed -and
        $result.stylePresent -is [bool] -and [bool]$result.stylePresent -and
        ($foregroundDocument -or $verifiedBackgroundWindow) -and
...
try {
  ...
  try {
    ...
    # Store activation can create the browser process well before its loopback
    # DevTools endpoint starts accepting requests. Cold starts are especially
    # slow immediately after a Codex update, so retain the verified launch for
    # a longer bounded window before rolling it back.
    ...
    while ($null -eq $cdpIdentity) {
      ...
      if ((Get-Date) -ge $deadline) {
        if ($null -ne $debugLaunch -and $debugLaunch.Strategy -eq 'direct-store-executable') {
          throw "The validated direct Store executable fallback did not expose a verified loopback CDP endpoint on port $Port within 90 seconds. Codex $($codex.Version) may disable CDP in this production runtime; no protected app files or permissions were changed."
        }
