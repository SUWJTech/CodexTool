...
function Initialize-DreamSkinThemeStore {
  ...
  # Seed every validated CodexTool preset. This makes the native catalog and
  # the installed engine use one source of truth instead of a hard-coded theme.
  $presetRoot = Join-Path $SkillRoot 'presets'
  $bundledPresetSources = @{}
  if (Test-Path -LiteralPath $presetRoot -PathType Container) {
    foreach ($presetSource in @(Get-ChildItem -LiteralPath $presetRoot -Directory)) {
      $sourcePack = Read-DreamSkinTheme -ThemeDirectory $presetSource.FullName
      $sourceId = "$($sourcePack.Theme.id)"
      if ($sourceId -cnotmatch '^preset-[A-Za-z0-9_-]{1,72}$') {
        throw "Bundled theme id must be a safe preset id: $sourceId"
      }
      $destination = Join-Path $paths.Saved $sourceId
      Ensure-DreamSkinManagedDirectory -Path $destination -Root $paths.Root
      $imageName = [System.IO.Path]::GetFileName($sourcePack.ImagePath)
      $destinationImage = Join-Path $destination $imageName
      Assert-DreamSkinNoReparseComponents -Path $destinationImage
      Assert-DreamSkinImageFile -Path $sourcePack.ImagePath
      Copy-Item -LiteralPath $sourcePack.ImagePath -Destination $destinationImage -Force
      Copy-Item -LiteralPath $sourcePack.ThemePath -Destination (Join-Path $destination 'theme.json') -Force
      $bundledPresetSources[$sourceId] = $presetSource.FullName
    }
  }
  ...
  if (Test-Path -LiteralPath $activeTheme -PathType Leaf) {
    ...
    elseif ($bundledPresetSources.ContainsKey($activeId)) {
      $refreshSource = $bundledPresetSources[$activeId]
    }
