...
function Confirm-DreamSkinCommunityApply {
  ...
  if ($CodexToolSilent) { return $true }
  Add-Type -AssemblyName System.Windows.Forms -ErrorAction Stop
...
function Assert-DreamSkinCommunityActiveBaseline {
  ...
  if (Test-DreamSkinPendingAppearanceTransaction -BackupPath $backupPath) {
    # The locked start path already owns the durable, three-way recovery for
    # interrupted appearance writes. Classify the stale journal like a missing
    # live session so one-click apply restarts Codex, recovers the transaction,
    # and verifies the renderer before replacing any theme files.
    $exception = [System.InvalidOperationException]::new(
      'One-click apply must recover the interrupted appearance transaction before verification.'
    )
...
try {
  ...
  $cleanupMessage = if ($workRootRetained) {
    ...
    # $workRoot belongs to Invoke-DreamSkinCommunityApply's local scope. Under
    # StrictMode it cannot be read from this outer catch block; doing so masked
    # the original apply failure with a VariablesIsUndefined error. The inner
    # finally block always removes non-retained work directories, so the
    # exception flag is the only state this handler needs.
