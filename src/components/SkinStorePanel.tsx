...
const COPY = {
  "zh-CN": {
    ...
    count: (count: number) => `${count} 个当前平台可用主题`,
    empty: "没有匹配的官方主题",
    safe: "本地引擎已随 CodexTool 自动部署。应用主题时会验证官方元数据、文件大小、SHA-256、ZIP 清单与 Safe CSS，并按需重启 Codex；不会修改 macOS 应用包、WindowsApps、app.asar、签名或 ACL。",
  ...
  "en-US": {
    ...
    count: (count: number) => `${count} themes for this platform`,
    empty: "No matching official themes",
    safe: "The local engine is deployed with CodexTool. Applying validates official metadata, size, SHA-256, the ZIP manifest, and Safe CSS before restarting Codex when needed; macOS app bundles, WindowsApps, app.asar, signatures, and ACLs remain untouched.",
    ...
    unsupported: "Native skinning is not supported on this platform.",
    downloads: "downloads",
  },
} as const;
